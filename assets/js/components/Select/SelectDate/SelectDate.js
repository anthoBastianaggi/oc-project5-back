...
import { range, isValidDate, dayPad, dateToValues } from "../../../utils/select-date-utils/select-date-utils";
    ...
    <div className={cls}>
      <div className="date-picker-container">
        {fields.map(field => (
            ...
            classNameError={validationError && showErrors ? "error" : ""}
            />
        ))}
      </div>
      {validationError && showErrors && <p className="error-message-date">{errors[validationError]}</p>}
    </div>
  );
};
